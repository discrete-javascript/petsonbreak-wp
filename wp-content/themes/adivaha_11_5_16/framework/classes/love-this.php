...
class mk_lovePost {
	...
	function love_post( $post_id, $action = 'get' ) {
		...
		case 'update':
			$love_count = get_post_meta( $post_id, '_mk_post_love', true );
			if ( isset( $_COOKIE['twc_theme_love_'. $post_id] ) ) return $love_count;

			$love_count++;
			update_post_meta( $post_id, '_mk_post_love', $love_count );
			setcookie( 'twc_theme_love_'. $post_id, $post_id, time()*20, '/' );
	...
	function send_love() {
		...
		$class = '';
		if ( isset( $_COOKIE['twc_theme_love_'. $post->ID] ) ) {
			$class = 'item-loved';
		}

		return '<a href="#" class="mk-love-this '. $class .'" id="mk-love-'. $post->ID .'"><i class="mk-icon-heart"></i> '. $output .'</a>';
	}
