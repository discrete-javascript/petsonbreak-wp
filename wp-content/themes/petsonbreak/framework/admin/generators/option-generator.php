...
<?php
    ...
    function start( $value ) {
		...
		echo '<li><a href="#mk_options_expedia"><i class="icon-blog"></i>'.__( 'EAN Settings', 'mk_framework' ).'</a></li>';
        ...
        //echo '<li><a href="#mk_options_advanced"><i class="icon-lab"></i>'.__( 'Advanced', 'mk_framework' ).'</a></li>';
		 echo '<li><a href="#mk_options_pagecontent"><i class="icon-lab"></i>'.__( 'Page Content', 'mk_framework' ).'</a></li>';
        echo '</ul>';
        echo '</div>';
        ...
        if ( !empty( $sites ) ) {
            foreach ( $sites as $site ) {
                echo '<div class="mk-social-item added-item">
                            <div class="social-item-icon"><i class="twc-theme-icon-simple-'.$site.'"></i></div>
                            <div class="social-item-url"></div>
                            <a href="#" class="delete-social-item"><i class="icon-close2"></i></a>
...
-------------------------------------------------------------*/
    function sys_diagnose( $value ) {
        ...
        echo '<a href="http://artbees.net/themes/twc/docs" target="_blank" class="mk-button dominant-color mk-secondary">Documentation</a>';
        echo '</div>';
    }
...
*/
  ...
  /*
	** Type : Text Box
-------------------------------------------------------------*/
   function text_city_script( ) {
        echo '<script type="text/javascript" src="//maps.googleapis.com/maps/api/js?sensor=false&libraries=places"></script>';

    }

    function text_city( $value ) {
        ...
        echo "<input name='" . $value['id'] . "' id='" . $value['id'] . "' type='text' class='mk-textbox load_city' size='" . $size . "' value='";
        if ( isset( $this->saved_options[$value['id']] ) ) {
            echo htmlspecialchars( $this->saved_options[$value['id']], ENT_QUOTES );
        }
        else {
		...
		google.maps.event.addDomListener(window, "load", function () {
			...
			var places = new google.maps.places.Autocomplete(document.getElementById("'.$value['id'].'"));
            google.maps.event.addListener(places, "place_changed", function () {
                var place = places.getPlace();
                var latitude = place.geometry.location.lat();
                var longitude = place.geometry.location.lng();
				document.getElementById("'.$value['id'].'_child").value = latitude+"^^^"+longitude; 				
...
-------------------------------------------------------------*/
    ...
    function text_city_child( $value ) {
        $size = isset( $value['size'] ) ? $value['size'] : '40';
        $option_structure = isset( $value['option_structure'] ) ? $value['option_structure'] : 'sub';
        $no_divider = isset( $value['divider'] ) ? 'with-divider' : 'no-divider';
        $value['divider'] = isset( $value['divider'] ) ? $value['divider'] : true;
        ...
        echo "<input name='" . $value['id'] . "' id='" . $value['id'] . "' type='hidden' class='mk-textbox' size='" . $size . "' value='";
        if ( isset( $this->saved_options[$value['id']] ) ) {
            echo htmlspecialchars( $this->saved_options[$value['id']], ENT_QUOTES );
        }
        else {
