...
if($_REQUEST["action"]=="findSearchKey"){
	...
	$moreQuery =" and is_pet=1 and currency='".$_SESSION['Cri_currency']."' and language='".$_SESSION['Cri_language']."' and LowRate>0";
	...
	(select count(EANHotelID) from search_results_ean where proximityDistance<=".$pd2." and search_session='".$search_Session_Id."' ".$moreQuery.") as distance2,
  (select count(EANHotelID) from search_results_ean where  proximityDistance<=".$pd5." and search_session='".$search_Session_Id."' ".$moreQuery.") as distance5,
  (select count(EANHotelID) from search_results_ean where proximityDistance<=".$pd10." and search_session='".$search_Session_Id."' ".$moreQuery.") as distance10,
  (select count(EANHotelID) from search_results_ean where proximityDistance<=".$pd20." and search_session='".$search_Session_Id."' ".$moreQuery.") as distance20,
  (select count(EANHotelID) from search_results_ean where proximityDistance<=".$pd50." and search_session='".$search_Session_Id."' ".$moreQuery.") as distance50,
...
if($_REQUEST["action"]=="Upldate_Rates_All"){
	...
	// controller
	$moreQuery =" is_pet=1 and currency='".$_SESSION['Cri_currency']."' and language='".$_SESSION['Cri_language']."' and LowRate>0";
	if(($_REQUEST['hotelType']!='') && ($_REQUEST['hotelType']!=1) ){
		$moreQuery=' and propertyCategory='.$_REQUEST['hotelType'];
	}
...
if($_REQUEST["action"]=="Searched_Hotels"){
	...
	$moreCod =" and is_pet=1 and currency ='".$_SESSION['Cri_currency']."' and language='".$_SESSION['Cri_language']."' and  LowRate>0";
	...
	if(($_REQUEST['Cri_Distance']!='') && ($_REQUEST['Cri_Distance']>0)){
		...
		$moreCod.= ' and proximityDistance <='.($_REQUEST['Cri_Distance']/$measurement);
