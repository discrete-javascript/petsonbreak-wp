...
if($_REQUEST['postType']!=''){
	...
	echo '<script>window.location.href="'.site_url().'/member-profile/?type=addpets"</script>';
}
...
<?php if($_REQUEST['action']=='edit'){?>
	<h5 class="recent_bookings">Add/Edit Pets</h5>
					...
					<label class="radio-inline">
						<span>Yes</span> <input type="radio" name="own_pets" id="own_pets1" value="Yes"
							<?php if ($all_meta_for_user['own_pets'][0] == 'Yes'){
								$checked ='checked="checked"'; echo $checked; } else{ $checked=''; } ?> >
					</label>
					<label class="radio-inline">
						<span>No</span> <input type="radio" name="own_pets" id="own_pets2" value="No"
							<?php if ($all_meta_for_user['own_pets'][0] == 'No'){
								$checked ='checked="checked"'; echo $checked; } else { $checked=''; } ?> >
					</label>
				</div>
			</li>
										...
										<div id="petDetails_<?php echo $p;?>" style="width:85%">
											...
											<div>
												<select name="petData[pet_age][]" value="" placeholder="Age">
													...
													<?php
													for($i=1;$i<=100;$i++){ 
														if($i==$pet_age[$p]){ $agselected='selected="selected"'; } else { $agselected='';} ?>
											...
											<div>
												<?php $breeLists =$wpdb->get_Results("select * from twc_pet_breedtype where pet_type='".$petType."'");
												?>
												<select name="petData[pet_breed][]" id="breedSelect_<?php echo $p;?>">
													<option value="" selected disabled>Breed</option>
													<?php
													foreach($breeLists as $breed){
														if($breed->id==$pet_breed[$p]){$bselected='selected="selected"';} else{$bselected='';}
											...
											<div class="deletePets" rel="<?php echo $p;?>">[Delete]</div>
										</div> 
									<?php } else{?>
										<div id="petDetailsSmall_<?php echo $p;?>" class="petDetailsSmall" style="width:85%; display:block;">
											<div>
...
<?php } else { ?>
	<h5 class="recent_bookings">Pets <span><a href="<?php echo site_url();?>/member-profile/?type=addpets&action=edit">Add/Edit Pets</a></span></h5>
	...
	<ul class="modalLogin-loginFields member-prof-details">
		...
		<li class="pets_info" <?php if($all_meta_for_user['own_pets'][0] == 'No') { echo 'style="display:none"'; } ?>>
			<div class="row">
				<div class="col-md-12">
...
<script>
	$('.del_image').click(function() {
	    ...
	    $.ajax({
	        type: "POST",
	        url: "<?php echo get_template_directory_uri(); ?>/custom-ajax.php",
	        data: "action=deleteImage&img_delete=" + rel + "&attr=" + attr,
	        success: function(Data) {
	            if (Data == 1) {
...
.modalLogin-loginFields li input {
    height: 40px;
    margin-bottom: 25px;
    width: 100%;
...
.modalLogin-loginFields li:nth-child(2n-2) {
    width: 49.5%;
    float: right;
}
...
#memberProf .petLista_list li div {
    width: 19%;
    float: left!important;
    margin-right: 1%;
}
...
#images_preview .deleteImage {
    margin: 0px 10px;
    font-size: 20px;
    color: red;
}

#memberProf .spayed_div_left {
    width: 49%;
    float: left;
}

#memberProf .spayed_div_right {
    width: 49%;
    float: right;
}

#memberProf #addMorePets {
    width: 100%;
    float: left;
    margin-bottom: 25px;
}

#memberProf #addMorePets span {
    background: red;
    color: #fff;
    padding: 10px 25px;
    font-size: 14px;
...
#memberProf .petLista_list li {
    width: 100% !important;
    float: left;
}

#memberProf .petLista_head li {
    width: 100% !important;
    padding-bottom: 10px;
}

#memberProf .deletePetss {
    width: 19%;
    float: left!important;
    margin-right: 1%;
    height: 40px;
