...
$headers = "MIME-Version: 1.0" . "\r\n";
	...
	$pmail_subject ='NEW CONTACT for Hotel booking';
	$pmailBody ='<table cellpadding="2" cellspacing="2">
	<p>Find the new query</p>
	<tr><td>Client name: '.$_REQUEST['name'].'</td></tr>
	<tr><td>Email: '.$_REQUEST['email'].'</td></tr>
	<tr><td>Contact No: '.$_REQUEST['contact_no'].'</td></tr>
	<tr><td>Subject: '.$_REQUEST['subject'].'</td></tr>
	<tr><td>No of Adutls: '.$_REQUEST['no_of_adults'].'</td></tr>
	<tr><td>No of Childs: '.$_REQUEST['no_of_child'].'</td></tr>
	<tr><td>Pet Type: '.$_REQUEST['pet_type'].'</td></tr>
	<tr><td>Check In Date: '.$_REQUEST['check_in_date'].'</td></tr>
	<tr><td>Check Out Date: '.$_REQUEST['check_out_date'].'</td></tr>
	<tr><td>message: '.$_REQUEST['message'].'</td></tr>
	</table>';
	
	
	$admin_email = 'siddhartha.ambekar@gmail.com';
	echo @mail($admin_email,$pmail_subject,$pmailBody,$headers);die();
...
if($_REQUEST['action']=='UserFeedback'){
 ...
 if($_REQUEST["email_id"]!=''){
	 ...
	 $name = explode("@",$_REQUEST['email_id']);
...
?>			
	   ...
	   <div class="slide_item bx-clone" style="float: left; list-style: outside none none; position: relative; width: 210px; margin-right: 10px;">         
	       			<div class=" col-md-4 col-sm-4 col-xs-4">
				<div class="det-sec2-col">
				<h5><?php echo $name[0]; ?></h5>
				<span class="star-ioncs<?php echo ceil($_REQUEST['scale_rate']/2); ?>"></span>
				<span class="bkd_from"><?php echo $_REQUEST['email_id']; ?></span>
				<span class="travld_Date"><?php echo date('l M d, Y',strtotime(date('Y-m-d H:i:s')));?></span>
				<p class="cont"><?php echo $_REQUEST['feedback_area']; ?> </p>
