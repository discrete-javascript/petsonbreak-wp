...
if (!function_exists('mk_new_version_migration')) {
     ...
     {
          $version = get_option('twc_theme_version');
          ...
          if($theme_data['Version'] != $version) {
           ...
           $theme_options = array();   
            ...
            update_option('twc_theme_version', $theme_data['Version']);
...
if (!function_exists('mk_flush_rules')) {
     ...
     {
          if (get_option('twc_theme_flush_rules')) {
               global $wp_rewrite;
               $wp_rewrite->flush_rules();
               delete_option('twc_theme_flush_rules');
          }
