	 ...
	 var filter_Data =$('#filter_frm').serialize();
	  $.ajax({
		 ...
		 for(a=0; a<myData.length; a++){
			str+='<li>';
				str+='<div class="float_left14" style="background-image:url(<?php echo get_template_directory_uri(); ?>/uploads/'+myData[a].image_path+');">';
				
				str+='</div>';
				str+='<div class="float_right14">';
					str+='<h2>'+myData[a].title+'</h2>';
					str+='<div class="text-pack17">';
					str+='<span><i class="fa fa-map-marker"></i>'+myData[a].address+'<span class="low_price1"></span></span>';
					str+='<span><i class="fa fa-wrench" aria-hidden="true"></i>'+myData[a].service_offered+'</span>';
				...
				//str+='<span><i class="fa fa-map-marker"></i>'+getFieldByID('title','twc_manage_offered',myData[a].service_offered)+'</span>';
					...
					/* str+='<span><i class="fa fa-phone"></i>';
					if(myData[a].on_call=='Yes'){
					str+=myData[a].contact;
					}
				    str+='<span class="price165">$'+myData[a].price+'</span></span>'; */
					
					str+='<span><i class="fa fa-credit-card" aria-hidden="true"></i>Card Accepted:('+myData[a].payments+')</span>';
