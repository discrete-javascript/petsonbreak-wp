...
if($_REQUEST['action']=='AddReview'){
	$flag=0;
	$sql ="Insert into twc_review SET id='".uniqid()."', title='".$_REQUEST['user_name']."', email='".$_REQUEST['user_email']."',user_id='".$_REQUEST['user_id']."',rating='".$_REQUEST['rating']."',product_id='".$_REQUEST['product_id']."',message='".$_REQUEST['message']."',published='No', date_time='".date('Y-m-d H:i:s')."'";
	$wpdb->query($sql);
	$flag=1;
	echo $flag;
}
...
if($_REQUEST['action']=='searchVendorServices'){
   ...
   if(@$_REQUEST['p']<=1){
	...
	foreach($resuts  as $obj){
		...
		$data[] =array('id'=>$obj->id,'vendor_id'=>$obj->vendor_id,'title'=>$obj->title,'description'=>$obj->description,'contact'=>$obj->contact,'address'=>$obj->address,'zipcode'=>$obj->zipcode,'city'=>$obj->city,'country'=>$obj->country,'time_from'=>$obj->time_from,'time_to'=>$obj->time_to,'payments'=>$obj->payments,'on_call'=>$obj->on_call,'service_category'=>$obj->service_category,'service_offered'=>$offerd,'price'=>$obj->price,'image_path'=>$obj->image_path,'latitude'=>$obj->latitude,'longitude'=>$obj->longitude,'avg_rating'=>$obj->avg_rating);
	}
	/* echo"<pre>";
	print_r($resuts);die; */
