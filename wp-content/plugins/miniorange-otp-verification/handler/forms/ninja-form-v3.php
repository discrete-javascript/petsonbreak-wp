<?php 
	...
	class NinJaFormAjaxForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_ninja_form_enable_type');
			$this->ninjaForms = maybe_unserialize(get_option('mo_customer_validation_ninja_form_otp_enabled'));
		...
		function enqueue_nj_form_script($form_id)
		{
			if(array_key_exists($form_id,$this->ninjaForms))
			{
				...
				wp_localize_script('njscript', 'moninjavars', array(
					...
					'key'     		=> 	$this->otpType=='mo_ninja_form_phone_enable' 
											? "nf-field-".$formdata['phonekey'] : "nf-field-".$formdata['emailkey'],
					'fieldName'		=> 	$this->otpType=='mo_ninja_form_phone_enable' 
											? "phone number" : "email",	
					'verifyField'	=>	get_option('mo_customer_validation_nfa_verify_field'),
					'siteURL' 		=> 	site_url(),
				));
		...
		{
			return get_option('mo_customer_validation_nja_enable') ? true : false;
		}
		...
		function _add_button($fields)
		{
			...
			$fieldName = $this->otpType=='mo_ninja_form_phone_enable' ? "phone number" : "email";
			$fieldKey = $this->otpType=='mo_ninja_form_phone_enable' ? "phonekey" : "emailkey";
		...
		function _handle_nj_ajax_form_submit($data)
		{
			...
			if(strcasecmp($this->otpType,'mo_ninja_form_email_enable')==0)
				$data = $this->processEmail($formdata,$data);
			if(strcasecmp($this->otpType,'mo_ninja_form_phone_enable')==0)
		...
		function _send_otp_nj_ajax_verify($data)
		{
			...
			if($this->otpType=="mo_ninja_form_phone_enable")
		...
		{
			$_SESSION[ strcasecmp($this->otpType,"mo_ninja_form_phone_enable")==0 ? $this->formPhoneVer : $this->formEmailVer ] = $sessionvalue;
		...
		function getEmailFieldId($data,$key)
		{
			global $wpdb;
			return $wpdb->get_var("SELECT id FROM {$wpdb->prefix}nf3_fields where `key` ='".$data['ninja_ajax_form']['emailkey'][$key]."'");
		}

		function getPhoneFieldId($data,$key)
		{
			global $wpdb;
			return $wpdb->get_var("SELECT id FROM {$wpdb->prefix}nf3_fields where `key` ='".$data['ninja_ajax_form']['phonekey'][$key]."'");
		}

		function getVerificationFieldId($data,$key)
		{
			global $wpdb;
			return $wpdb->get_var("SELECT id FROM {$wpdb->prefix}nf3_fields where `key` ='".$data['ninja_ajax_form']['verifyKey'][$key]."'");
		}
		...
		function handleFormOptions()
		{
			...
			{
				$form[$value]= array(
					'emailkey'=> $this->getEmailFieldId($_POST,$key),
					'phonekey'=> $this->getPhoneFieldId($_POST,$key),
					'verifyKey'=> $this->getVerificationFieldId($_POST,$key),
			...
			update_option('mo_customer_validation_ninja_form_enable',0);
			update_option('mo_customer_validation_nja_enable',
				isset( $_POST['mo_customer_validation_nja_enable']) ? $_POST['mo_customer_validation_nja_enable'] : 0 );
			update_option('mo_customer_validation_ninja_form_enable_type',
				isset( $_POST['mo_customer_validation_nja_enable_type']) ? $_POST['mo_customer_validation_nja_enable_type'] : '');
			update_option('mo_customer_validation_ninja_form_otp_enabled',!empty($form) ? maybe_serialize($form) : "");
		}
	}
