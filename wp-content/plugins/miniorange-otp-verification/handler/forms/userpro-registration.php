<?php
	...
	class UserProRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_userpro_enable_type');
			if(strcasecmp($this->otpType,'mo_userpro_registration_phone_enable')==0)
			{
		...
		{
			return get_option('mo_customer_validation_userpro_default_enable') ? true : false;
		}

		function _auto_verify_user($user_id)
		{
			if(get_option('mo_customer_validation_userpro_verify')) update_user_meta($user_id,'userpro_verified', 1);
		}
		...
		function _userpro_phone_shortcode()
		{
			...
			$htmlcontent  	.= "title='Please Enter a phone number to enable this.'>Click Here to Verify Phone</button></div>";
		...
		function _userpro_email_shortcode()
		{
			...
			$htmlcontent 	= "<div style='margin-top: 2%;'><button type='button' class='button alt' style='width:100%;height:30px;";
			$htmlcontent   .= "font-family: Roboto;font-size: 12px !important;' id='miniorange_otp_token_submit' ";
			$htmlcontent   .= "title='Please Enter a email to enable this.'>Click Here to Verify Email</button></div>";
		...
		function handleFormOptions()
	    {
			...
			update_option('mo_customer_validation_userpro_default_enable',
