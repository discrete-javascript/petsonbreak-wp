<?php
	...
	class BuddyPressRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->phoneKey = get_option('mo_customer_validation_bbp_phone_key');
			$this->otpType = get_option('mo_customer_validation_bbp_enable_type');
			$this->autoActivate = get_option('mo_customer_validation_bbp_default_enable');
			$this->disableAutoActivate = get_option('mo_customer_validation_bbp_disable_activation');
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$otpVerType = strcasecmp($this->otpType,"mo_bbp_phone_enable")==0 ? "phone" 
							: (strcasecmp($this->otpType,"mo_bbp_both_enable")==0 ? "both" : "email" );
		...
		{
			if(strcasecmp($this->otpType,"mo_bbp_phone_enable")==0)
				miniorange_site_challenge_otp($username,$email,$errors,$phone_number,'phone',$password,$extra_data);
			else if(strcasecmp($this->otpType,"mo_bbp_both_enable")==0)
