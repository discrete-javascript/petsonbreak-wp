<?php
	...
	class UltimateMemberRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_um_enable_type');
		...
		{
			if(strcasecmp($this->otpType,"mo_um_phone_enable")==0)
				miniorange_site_challenge_otp($username,$email,$errors,$phone_number,"phone",$password,$extra_data);
			elseif(strcasecmp($this->otpType,"mo_um_both_enable")==0)
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$otpVerType = strcasecmp($this->otpType,"mo_um_phone_enable")==0 ? "phone" 
							: (strcasecmp($this->otpType,"mo_um_both_enable")==0 ? "both" : "email" );
		...
		function handleFormOptions()
	    {
			...
			update_option('mo_customer_validation_um_default_enable',
				isset( $_POST['mo_customer_validation_um_default_enable']) ? $_POST['mo_customer_validation_um_default_enable'] : 0);
			update_option('mo_customer_validation_um_enable_type',
