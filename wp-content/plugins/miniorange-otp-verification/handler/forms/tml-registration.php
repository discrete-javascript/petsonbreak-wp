<?php
	...
	class TmlRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_tml_enable_type');
		...
		{	
	 		...
	 		if( $this->otpType == 'mo_tml_phone_enable' || $this->otpType == 'mo_tml_both_enable')
		...
		function miniorange_tml_registration_errors($errors, $sanitized_user_login, $user_email )
		{
			...
			elseif($this->otpType == 'mo_tml_email_enable')
				$this->startOTPTransaction($sanitized_user_login,$user_email,$errors,$phone_number);
			else
		...
		function startOTPTransaction($sanitized_user_login,$user_email,$errors,$phone_number)
		{
			...
			if(strcasecmp($this->otpType,"mo_tml_phone_enable")==0)
				miniorange_site_challenge_otp($sanitized_user_login,$user_email,$errors,$phone_number,"phone");
			else if(strcasecmp($this->otpType,"mo_tml_both_enable")==0)
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$otpVerType = strcasecmp($this->otpType,"mo_tml_phone_enable")==0 ? "phone" 
							: (strcasecmp($this->otpType,"mo_tml_both_enable")==0 ? "both" : "email" );
		...
		function handleFormOptions()
	    {
			...
			update_option('mo_customer_validation_tml_enable',
				isset( $_POST['mo_customer_validation_tml_enable']) ? $_POST['mo_customer_validation_tml_enable'] : 0);
			update_option('mo_customer_validation_tml_enable_type',
