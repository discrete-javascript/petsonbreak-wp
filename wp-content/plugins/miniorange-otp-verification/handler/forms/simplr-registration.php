<?php
	...
	class SimplrRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->phoneFieldKey = get_option('mo_customer_validation_simplr_field_key');
			$this->otpType = get_option('mo_customer_validation_simplr_enable_type');
	    ...
	    function simplr_site_registration_errors($errors)
	    {
			...
			if(isset($errors) || isset($_POST['fbuser_id'])) return $errors;
			...
			if(!$this->processPhone($phone_number,$errors)) return $errors;
		...
		function processAndStartOTPVerificationProcess($username,$email,$errors,$phone_number,$password,$extra_data)
		{
			...
			if(strcasecmp($this->otpType,"mo_phone_enable")==0)
				miniorange_site_challenge_otp($username,$email,$errors,$phone_number,"phone",$password,$extra_data);
			else if(strcasecmp($this->otpType,"mo_both_enable")==0)
