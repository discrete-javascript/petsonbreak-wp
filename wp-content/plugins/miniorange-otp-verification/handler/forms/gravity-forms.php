<?php
	...
	class GravityForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_gf_contact_type');
			$this->gfForms = maybe_unserialize(get_option('mo_customer_validation_gf_otp_enabled'));
		...
		{
			return get_option('mo_customer_validation_gf_contact_enable') ? true : false;
		}
		...
		{
			if(strcasecmp($this->otpType,"mo_gf_contact_phone_enable")==0)
			...
			{   
				foreach($this->gfForms[0] as $f)
					if($f==$form_id && get_class($field)=="GF_Field_Phone")
						$field_content = $this->_add_shortcode_to_form("phone",$field_content,$field);
			}
			else												
			{	
				foreach($this->gfForms[1] as $f)
					if($f==$form_id && get_class($field)=="GF_Field_Email")
						$field_content = $this->_add_shortcode_to_form("email",$field_content,$field);
			}
			return $field_content;	
		}

		function _add_shortcode_to_form($mo_type,$field_content,$field) 
		{
			...
			$field_content .= 'var e=$mo("input[name=input_'.$field->id.']").val(); $mo("#mo_message").empty(),$mo("#mo_message").append("'.$img.'")';
		...
		{  
		    if($this->otpType=="mo_gf_contact_phone_enable")
				foreach($this->gfForms[0] as $f)
		...
		function validate_otp($field,$error,$f,$value)
		{
			...
			if($this->otpType=="mo_gf_contact_phone_enable" && strpos($field->label, 'Phone') !== false)
				$error = $this->validate_submitted_email_or_phone($error['is_valid'],$value,$error);

			if($this->otpType=="mo_gf_contact_email_enable" && strpos($field->label, 'Email') !== false)
				$error = $this->validate_submitted_email_or_phone($error['is_valid'],$value,$error);
