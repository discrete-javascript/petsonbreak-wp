<?php
	...
	class WPLoginForm extends FormInterface
	{
		...
		function handleForm()
		{	
			$this->phoneNumberKey = get_option('mo_customer_validation_wp_login_key');
		...
		{
			return get_option('mo_customer_validation_wp_login_enable') ? true : false;
		}

		function check_wp_login_register_phone() 
		{
			return get_option('mo_customer_validation_wp_login_register_phone') ? true : false;
		}

		function check_wp_login_bypass_admin()                                 
		{
			return get_option('mo_customer_validation_wp_login_bypass_admin') ? true : false;
		}

		function check_wp_login_by_phone_number()                                 
		{
			return get_option('mo_customer_validation_wp_login_allow_phone_login') ? true : false;
		}
		...
		{
			return get_option('mo_customer_validation_wp_login_restrict_duplicates') ? true : false;
		}
		...
		function getUserIfUsernameIsPhoneNumber($user,$username,$password,$key)
		{
			...
			return wp_authenticate_username_password(NULL,$user_info->data->user_login,$password);
		}

		function getUserFromPhoneNumber($username,$key)
		{
			global $wpdb;
			$results = $wpdb->get_row("SELECT `user_id` FROM `wp_usermeta` WHERE `meta_key` = '$key' AND `meta_value` =  '$username'");			
			return get_userdata($results->user_id);
		}
		...
		function askPhoneAndStartVerification($user,$key,$username,$phone_number)
		{
			...
			{		
				...
				mo_external_phone_validation_form(MoUtility::currentPageUrl(),$user->data->user_login,
													 MoMessages::showMessage('REGISTER_PHONE_LOGIN'),$key,array('user_login'=>$username));
			}					
		}
		...
		function fetchPhoneAndStartVerification($user,$key,$username,$password,$phone_number)
		{
			...
			miniorange_site_challenge_otp($username,null,null,$phone_number[0],"phone",$password,$_REQUEST['redirect_to'],false);
		}
