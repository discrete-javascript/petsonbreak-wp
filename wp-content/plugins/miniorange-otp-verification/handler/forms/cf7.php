<?php
	...
	class ContactForm7 extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_cf7_contact_type');
			$this->emailFieldKey = get_option('mo_customer_validation_cf7_email_key');
		...
		function _handle_cf7_contact_form($getdata)
		{
			...
			{
				if(strcasecmp($this->otpType,"mo_cf7_contact_phone_enable")==0)
		...
		function validateFormPost($result, $tag)
		{
			...
			if ( 'text' == $tag->basetype && $name=='email_verify' || 'text' == $tag->basetype && $name=='phone_verify') 
			{
				$_SESSION[$this->formSessionTagName] = $name;
					...
					if($this->checkIfVerificationNotStarted()) $result->invalidate( $tag, MoMessages::showMessage('ENTER_VERIFY_CODE') );
					//check if the email being sent is the same email that was verified
					if($this->processEmail()) $result->invalidate( $tag, MoMessages::showMessage('EMAIL_MISMATCH') );
					//check if the phone being sent is the same phone that was verified
					if($this->processPhoneNumber()) $result->invalidate( $tag, MoMessages::showMessage('PHONE_MISMATCH') ); 
