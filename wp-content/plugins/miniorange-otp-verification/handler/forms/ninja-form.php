<?php 
	...
	class NinjaForm extends FormInterface
	{
		...
		function handleForm()
		{
			...
			$this->ninjaFormsEnabled = maybe_unserialize(get_option('mo_customer_validation_ninja_form_otp_enabled'));
		...
		function _handle_ninja_form_submit($requestdata) 
		{
			...
			$email = $this->processEmail($formdata,$requestdata); 			$phone = $this->processPhone($formdata,$requestdata); 			$this->miniorange_ninja_form_user($email,null,$phone);
		}

		function processPhone($formdata,$requestdata)
		{
			if(strcasecmp($this->otpType,'mo_ninja_form_phone_enable')==0 
				|| strcasecmp($this->otpType,'mo_ninja_form_both_enable')==0)
			{
		...
		function miniorange_ninja_form_user($user_email,$user_name,$phone_number)
		{
			...
			else if(strcasecmp($this->otpType,"mo_ninja_form_both_enable")==0)
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$otpVerType = strcasecmp($this->otpType,"mo_ninja_form_phone_enable")==0 ? "phone" 
							: (strcasecmp($this->otpType,"mo_ninja_form_both_enable")==0 ? "both" : "email" );
		...
		function handleFormOptions()
		{
			...
			update_option('mo_customer_validation_ninja_form_enable',
				isset( $_POST['mo_customer_validation_ninja_form_enable']) ? $_POST['mo_customer_validation_ninja_form_enable'] : 0);
			update_option('mo_customer_validation_nja_enable',0);
