	...
	class WooCommerceRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			...
			add_filter('woocommerce_process_registration_errors', array($this,'woocommerce_site_registration_errors'),99,4);
						if($this->isPhoneVerificationEnabled()) add_action( 'woocommerce_register_form', array($this,'mo_add_phone_field') );
		}
		...
		{
			return (strcasecmp($this->otpType,"mo_wc_phone_enable")==0 || strcasecmp($this->otpType,'mo_wc_both_enable')==0);
		}
		...
		function processFormFields($username,$email,$errors,$password)
		{
			...
			else if(strcasecmp($this->otpType,"mo_wc_email_enable")==0)
			{
			...
			else if(strcasecmp($this->otpType,'mo_wc_both_enable')==0)
			{
				if ( !isset( $_POST['billing_phone'] ) || !MoUtility::validatePhoneNumber($_POST['billing_phone']))
					return new WP_Error( 'billing_phone_error',
						str_replace("##phone##",$_POST['billing_phone'],$phoneLogic->_get_otp_invalid_format_message()) );
		...
		function mo_add_phone_field()
		{
			echo '<p class="form-row form-row-wide">
					<label for="reg_billing_phone">'.__( 'Phone', MoConstants::TEXT_DOMAIN ).'<span class="required">*</span></label>
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$otpVerType = strcasecmp($this->otpType,"mo_wc_phone_enable")==0 ? "phone" 
