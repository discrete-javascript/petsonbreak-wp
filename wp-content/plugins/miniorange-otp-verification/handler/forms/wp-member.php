<?php
	...
	{	
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_wp_member_reg_enable_type');
		...
		{
			return get_option('mo_customer_validation_wp_member_reg_enable') ? true : false;
		}
		...
		{
			if(strcasecmp($this->otpType,"mo_wpmember_reg_phone_enable")==0)
		...
		function wpmember_add_button($rows, $tag)
		{
			...
			{
				if(strcasecmp($this->otpType,"mo_wpmember_reg_phone_enable")==0 && $key=="phone1")
				{
					$rows[$key]['field'] .= $this->_add_shortcode_to_wpmember("phone",$field['meta']);
					break;
				}
				else if(strcasecmp($this->otpType,"mo_wpmember_reg_email_enable")==0 && $key=="user_email")
				{
		...
		function handleFormOptions()
		{
			...
			update_option('mo_customer_validation_wp_member_reg_enable',
				isset( $_POST['mo_customer_validation_wp_member_reg_enable']) ? $_POST['mo_customer_validation_wp_member_reg_enable'] : 0);
			update_option('mo_customer_validation_wp_member_reg_enable_type',
