<?php
	...
	class ClassifyRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_classify_type');
		...
		{
			return get_option('mo_customer_validation_classify_enable') ? TRUE : FALSE;
		}
		...
		{
			wp_enqueue_script('script', MOV_URL . 'includes/js/classify.js?version='.MOV_VERSION , array('jquery'));
		}
		...
		function _handle_classify_theme_form_post($data)
		{
			...
			if(strcasecmp($this->otpType,'classify_phone_enable')==0)
				miniorange_site_challenge_otp($_POST['username'],$email_id,null,$phone,"phone",null,null);
			else if(strcasecmp($this->otpType,'classify_email_enable')==0)
		...
		function handle_failed_verification($user_login,$user_email,$phone_number)
		{
			...
			$otpVerType = strcasecmp($this->otpType,"classify_phone_enable")==0 ? "phone" 
							: (strcasecmp($this->otpType,"classify_email_enable")==0 ? "email" : "both" );
		...
		function handleFormOptions()
		{
			...
			update_option('mo_customer_validation_classify_enable',
				isset( $_POST['mo_customer_validation_classify_enable']) ? $_POST['mo_customer_validation_classify_enable'] : 0);
			update_option('mo_customer_validation_classify_type',
