...
class EmailLogic extends LogicInterface
{
	...
	{
		$content = json_decode(MocURLOTP::mo_send_otp_token('EMAIL',$user_email), true);
	...
	public function _handle_otp_sent($user_login,$user_email,$phone_number,$otp_type,$from_both,$content)
	{
		...
		if(MoUtility::micr() && !MoUtility::mclv()) update_option('mo_customer_email_transactions_remaining',
												get_option('mo_customer_email_transactions_remaining')-1);
	...
	{
		return get_option("mo_otp_success_email_message") ? get_option('mo_otp_success_email_message') : MoMessages::showMessage('OTP_SENT_EMAIL');
	}

	public function _get_otp_sent_failed_message()
	{
		return get_option("mo_otp_error_email_message") ? get_option('mo_otp_error_email_message') : MoMessages::showMessage('ERROR_OTP_EMAIL');
	}
	...
	{
		return get_option("mo_otp_blocked_email_message") ? get_option('mo_otp_blocked_email_message') : MoMessages::showMessage('ERROR_EMAIL_BLOCKED');
	}
