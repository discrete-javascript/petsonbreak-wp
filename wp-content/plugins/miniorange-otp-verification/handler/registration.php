...
class MoRegistrationHandler
{
	...
	function handle_customer_registration()
	{
		...
		switch($option)
		{
			...
			case "mo_registration_resend_otp":
				$this->_send_otp_token(get_option('mo_customer_validation_admin_email'),"",'EMAIL');  	break;
	...
	{
		update_option( 'mo_customer_validation_admin_customer_key'  , $id 		);
		update_option( 'mo_customer_validation_admin_api_key'       , $apiKey   );
		update_option( 'mo_customer_validation_customer_token'		, $token 	);
		delete_option( 'mo_customer_validation_verify_customer'				  	);
		delete_option( 'mo_customer_validation_new_registration'			  	);
	...
	function _validate_otp($post)
	{
		...
		$content = json_decode(MocURLOTP::validate_otp_token(get_option('mo_customer_validation_transactionId'), $otp_token ),true);
		if(strcasecmp($content['status'], 'SUCCESS') == 0)
		{
			...
			else if(strcasecmp($customerKey['status'], 'SUCCESS') == 0)
			{
				...
				update_option('mo_customer_email_transactions_remaining',MoConstants::EMAIL_TRANS_REMAINING);
				update_option('mo_customer_phone_transactions_remaining',MoConstants::PHONE_TRANS_REMAINING);
