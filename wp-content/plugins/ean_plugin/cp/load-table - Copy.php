<?php session_start();
	...
	closedir($handle);
		...
		$items = $doc->getElementsByTagName("Response");
			...
			if(($_REQUEST["fn"]=="4")&&($_REQUEST['catID']!="")){
			$query_pag_num1 .= " and genre_category='".$_REQUEST['catID']."'";	
			}
			...
			if($Flds[$a+4]!='')
			{
				...
				if(($_REQUEST["fn"]=="4")&&($_REQUEST['catID']!="")){
			$order_val .= " and genre_category='".$_REQUEST['catID']."'";	
			}
  ...
  <tr>
    <td colspan="7">
        ...
        <!--onfocus="Show_This(this);"-->
            ...
            <?php
			if($_REQUEST["fn"]==4){
			  echo '<div class="fl select"><strong>Category</strong>';
			  $results_user_arr =$wpdb->get_results("select * from tbl_genre_category WHERE published='Yes' and status_deleted=0 order by title ASC");
			  echo '<select name="category" id="category">
                     <option value="" selected="selected">All</option>';
					 foreach($results_user_arr as $results_user){
						if($_REQUEST['catID']==$results_user->id){$selected ='selected="selected"';}
						else{$selected='';}
...
//Initials Required End
			...
			if(($_REQUEST["fn"]=="4")&&($_REQUEST['catID']!="")){
			$sql .= " and genre_category='".$_REQUEST['catID']."'";	
			}
		...
		$msg="";
			...
			/* --------------------------------------------- */
if(($_REQUEST["fn"]=="4")&&($_REQUEST['catID']!="")){
$addQuery .= " and genre_category='".$_REQUEST['catID']."'";	
}

$query_pag_num = "SELECT COUNT(".$Flds[0].") AS counts FROM ".$table_name." where status_deleted=0 ".$Publisheds.$addQuery ;
